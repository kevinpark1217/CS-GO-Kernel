...
#define KBDCLASS_CONNECT_REQUEST 5853
...
NTSTATUS Keyboard_Create(IN PDRIVER_OBJECT driverObject)
{
	...
	RtlInitUnicodeString(&uniKbdDeviceName,L"\\Device\\BarbellKeyboard");
...
NTSTATUS Keyboard_Hook(IN PDRIVER_OBJECT driverObject)
{
	...
	status = IoGetDeviceObjectPointer( &uniKbdDeviceName, FILE_ALL_ACCESS, &KbdFileObject,	&KbdDeviceObject );
